...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+mysqlconnector://root:@localhost/tender"
...
def home():
    ...
    return render_template('index.html',header = "loginHeader.html",menu = "menu.html",notices = notices,statuses = tenderstatus)
...
def createTender():
    if('user' in session and session['user'] == params['admin_user']):
        if(request.method == 'POST'):
            ...
            newTender = Tender(tname = tname,ttype = ttype,tprice = tprice,tdeadline = tdeadline,tloc = tloc,tdesc = tdesc)
            db.session.add(newTender)
            db.session.commit()
            return redirect("/viewTender")
        return render_template('createTender.html',header = "adminHeader.html",menu = "adminMenu.html")
...
def bidTender():
    ...
    tenderstatus = Tenderstatus.query.all()
    if('user' in session):
        return render_template('bidTender.html',header = "header.html",menu = "vendorMenu.html",tenders = tender,statuses = tenderstatus,notices = notices)
    else:
        return redirect('/loginFailed')
...
def viewAssignedTenders():
    ...
    notices = Notice.query.all()
    tender = Tender.query.all()
    return render_template('viewAssignedTenders.html',header = "adminHeader.html",menu = "adminMenu.html",notices = notices,statuses = tenderstatus,tenders = tender)
...
def viewTenderBids():
    return render_template('viewTenderBids.html',header = "adminHeader.html",menu = "adminMenu.html")
